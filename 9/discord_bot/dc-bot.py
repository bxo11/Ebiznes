...
@bot.command(name='ai')
async def chat_with_ai(ctx, message):
    url = f'{backend_url}/ai'
    id = ctx.author.name
    if id not in messages_to_ai:
        messages_to_ai[id] = [{'message': message, 'author': 'user'}]
    else:
        messages_to_ai[id].append({'message': message, 'author': 'user'})

    product_data = {
        "messages": messages_to_ai[id],
    }
    response = requests.post(url, json=product_data).json()
    assistant_message = response['message']['content']
    messages_to_ai[id].append({'message': assistant_message, 'author': 'assistant'})
